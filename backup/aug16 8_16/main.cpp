                        ...
                        if (csv.available())
                        {
                            ...
                            while (csv.available())
                            {
                                ...
                                if (EmpId != "")
                                {
                                    ...
                                    http.begin("http://13.126.195.214:8080/EmployeeAttendenceAPI/employee/EmployeeCheckInOut");
...
void updateEmployeeDetails(void *pvParameters)
{
  ...
  while (true)
  {
    ...
    Serial.println("5 hours passed. Starting update...");

    if (WiFi.status() == WL_CONNECTED)
    {
      Serial.println("Connected to Wi-Fi. Removing files...");
  ...
  if (WiFi.status() == WL_CONNECTED)
  {
    if (client.connect("www.google.com", 80))
    {
      ...
      do
      {
        ...
        http.begin("https://wildfly.tictoks.in:443/EmployeeAttendenceAPI/BiometricAPI/SelectAllEmployeeInfo");
        esp_task_wdt_reset();
        http.addHeader("Content-type", "application/json");
...
void DeviceidFetch()
{
  ...
  if (WiFi.status() == WL_CONNECTED)
  {
    if (client.connect("www.google.com", 80))
    {
      ...
      http.begin("https://wildfly.tictoks.in:443/EmployeeAttendenceAPI/BiometricAPI/GetBiometricUnMappedDeviceList");
      yield();
      http.addHeader("Content-type", "application/json");
...
void WebServerRoutes()
{
  ...
  server.on("/Device_id", HTTP_POST, [](AsyncWebServerRequest *request) {}, NULL, [](AsyncWebServerRequest *request, uint8_t *data, size_t len, size_t index, size_t total)
        ...
        if (json.success())
        {
          ...
          http.begin("https://wildfly.tictoks.in:443/EmployeeAttendenceAPI/BiometricAPI/BioMetricDoorMapping");
          http.addHeader("Content-type", "application/json");
  ...
  server.on("/Wifi_submit", HTTP_POST, [](AsyncWebServerRequest *request) {}, NULL, [](AsyncWebServerRequest *request, uint8_t *data, size_t len, size_t index, size_t total)
    ...
    while (WiFi.status() != WL_CONNECTED) {
      ...
      WifiConnectCheck();
      if(count>3){
        request->send(404, "text/plain", "WiFi Submited But Could not connect");
        break;
      }
      esp_task_wdt_reset();
      count++;
...
int getFingerprintID()
{
  ...
  if (p != FINGERPRINT_OK)
  {
    p = 0;
    Serial.println("Failed to find a match");
    return -2; // Use -1 to indicate failure
  }
  else{
  ...
  for (uint8_t reader = 0; reader < NO_OF_READERS; reader++)
  {
    ...
    if (mfrc522[reader].PICC_IsNewCardPresent() && mfrc522[reader].PICC_ReadCardSerial())
    {
      ...
      else
      {
        if (content.length() > 0)
        {
          Serial.println("Inside the Matchrfid");
