...
String ValidationChecker(int fingerid){
  ...
  if(SPIFFS.begin(true)){
    String Empid;//    SPIFFS.open("/EmpRfid.csv", "r");
    File readstrings = SPIFFS.open("/EmpRfid.csv", "r");
    if (!readstrings) {
            Serial.println("Failed to open file for reading");
          ...
          if (p == FINGERPRINT_OK) {
            esp_task_wdt_reset();
              ...
              if (p == FINGERPRINT_OK) {
                ...
                request->send(404, "text/html", "Fingerprint detected to Employee :"+Empid);
                return;
              } else {
  ...
  server.on("/re-registering", HTTP_POST, [](AsyncWebServerRequest *request) {
    ...
    if (status > 0) {
        ...
        if (p == FINGERPRINT_OK) {
            ...
            /*
            File datafiler = SPIFFS.open("/EmpRfid.csv", "r");
            if (!datafiler) {
              Serial.println("Failed to open file for reading");
              return;
            }
            while (datafiler.available()) {
              String line = datafiler.readStringUntil('\n');
              Serial.println(line);
            }
            datafiler.close();
            */
            ...
            File empidFile = SPIFFS.open("/EmpRfid.csv", "a");
            if (!empidFile) {
            ...
            */
            ...
            // String FingerData = downloadFingerprintTemplate(id);
            // Serial.println("Finger Print Data: ");
            // Serial.println(FingerData);
            // FingerData.replace(",", "");  // Remove commas
            // FingerData.trim();
            // char hexCharArray[FingerData.length() + 1];
            // FingerData.toCharArray(hexCharArray, sizeof(hexCharArray));
            // // Optionally, add code to write to file or other actions
            // size_t length = FingerData.length() / 2;
            // uint8_t byteArray[length];
            // hexStringToByteArray(hexCharArray, byteArray, length);
...
void fingerSensor(){
  ...
  if(FingerValidCheck){
    ...
    if (FingerPrintId <= 0)
    {
      Serial.print("Finger is not Registed");
      Serial.println(FingerPrintId);
      FingerAlreadyExist = true;
      FingerValidCheck = false;
    }
    else{
      Serial.print("Finger print is already registered:");
      Serial.println(FingerPrintId);
      FingerAlreadyExist = false;
    }
    if(fingerCountsensor >100){
      FingerValidCheck = false;
    }
    fingerCountsensor++;
  }
